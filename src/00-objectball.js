...
function numPointsScored(name) {
    let game = gameObject();
    for (let gameKey in game) { // home or away
        let teamObj = game[gameKey];
        for (let nameKey in teamObj.players) {
            if (nameKey === name) {
    ...
    return "Player is not on either team.";
}
    ...
    for (let gameKey in game) { // home or away
        ...
        if (teamObj.teamName === teamName) {
            for (let player in teamObj.players) {
                teamNumbers.push(teamObj.players[player].number);            
            }
        }
    }
    return teamNumbers;
